...
dirR <- setwd("~/Colgate/junior year/spring semester/env data science/activities/activity 7")
...
#Let's filter out all values in the rasters in a cloud area above 60% so that they are changed to a NA otherwise the value stays the same. 
#set clouds to NA
allbandsCloud <- list()
for(i in 1:9){
...
#ON WORD DOC
rf_errorM$byClass
#shows accuracy for each class but we can also use the table looking at the predictions
...
nnet_model
#make predictions for entire image
...
#QUESTION 3 (no coding just show both maps)
#ON WORD DOC

#Analyzing predictions of landcover
...
#QUESTION 4 (find area differences)
#ON WORD DOC

#QUESTION 5 (create a raster)

both<- overlay(nnet_prediction,rf_prediction,fun=function(x, y){return(x==y)})
plot(both,legend=FALSE, axes=FALSE)
mtext("Neural Network and Random Forest Overlay", side=3,cex=2, line=-5)

#QUESTION 6 (no coding)
#ON WORD DOC

#QUESTION 7 (no coding)
#ON WORD DOC

#QUESTION 8 (no coding)
#ON WORD DOC

#QUESTION 9 (no coding)
#ON WORD DOC

#QUESTION 10 (add to hithub)
#ON WORD DOC
